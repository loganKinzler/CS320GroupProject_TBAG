...
public class CommandsController {
	...
	public static String moveCommand(DerbyDatabase db, List<String> foundCommands, List<String> params, List<Room> rooms, PlayerController player, String systemResponse) {
	...
	public static String useCommand(IDatabase db, List<String> foundCommands, List<String> params, List<Room> rooms, String systemResponse, PlayerController player) {
		...
		if((params.get(1).equals("north") || params.get(1).equals("south") || params.get(1).equals("east")  || params.get(1).equals("west"))) {
			...
			Integer nextRoom = rooms.get(player.getCurrentRoomIndex() - 1).getConnectedRoom(params.get(1));
			if(rooms.get(nextRoom - 1).getRoom_key().equals(params.get(0))) {
				db.UpdateLockedRoom(nextRoom, "none");
				//systemResponse = String.format("Used %s...", params.get(0));
				systemResponse = String.format("The room is now unlocked!!");
			}
			
			else if(rooms.get(nextRoom - 1).getRoom_key().equals("none")) {
				systemResponse = String.format("The room has already been unlocked...");
			}
			
			else {
				systemResponse = String.format("Hmmm...My %s doesn't seem to fit in the key hole. Maybe I should try a differnt item.", params.get(0));
			}
	...
	public static String pickupCommand(
		...
		responseOut = ASCIIOutput.ItemAscii(servlet, params.get(1));
